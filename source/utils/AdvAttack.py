...
def compute_iou_acc(pred, target, n_cls, verbose=False, ignore_index=255, device=None):
    ...
    if verbose:
        print(f"mAcc={m_acc:.2%} aAcc={a_acc:.2%}", f" mIoU={m_iou:.2%}")
...
def js_loss(p, q, num_classes=21, reduction="mean"):
...
def apgd_train(
    ...
    if not use_rs:
        ...
        if norm == "Linf":
            ...
            x_adv = (x.clone() + eps * t).clamp(0.0, 1.0)
    if x_init is not None:
        # logger.log("Using specified initial point.")
        x_adv = x_init.clone()
    ...
    if loss_name not in ["orth-ce-avg"]:
        loss_indiv = criterion_indiv(logits, y, weights)
        ...
        if track_loss in [
            "mask-norm-corrlog-avg-targeted",
            "norm-corrlog-avg-targeted",
        ]:
    ...
    for i in range(n_iter):
        ...
        if True:  # with torch.no_grad()
            ...
            if norm == "Linf":
                ...
                x_adv_1 = x_adv + step_size * grad / (
                    L2_norm(grad, keepdim=True) + 1e-12
                )
                x_adv_1 = torch.clamp(
                    x
                    + (x_adv_1 - x)
                    / (L2_norm(x_adv_1 - x, keepdim=True) + 1e-12)
                    * torch.min(
        ...
        if verbose:
            str_stats = (
                ...
                if norm in ["L1"]
                else f" - step size: {step_size.mean():.5f}"
            )
            str_perf = f"mAcc={m_acc:.2%} aAcc={a_acc:.2%} mIoU={m_iou:.2%}"
            logger.log(
                "iteration: {} - best loss: {:.6f} curr loss {:.6f} - {}{}".format(  # robust accuracy: {:.2%}
                    i,
                    loss_best.sum(),
                    loss_curr,  # acc.float().mean(),
                    str_perf,
                    str_stats,
                )
            )
        ...
        if acc.sum() > 0:
            if "targeted" in loss:
                ...
                other_target = (
                    ...
                    if i == 0
                    else outputsorted[:, -target_cls + 1]
                )
...
def apgd_largereps(
    ...
    logger = Logger(log_path)
    n_iters = [int(c * n_iter) for c in [0.3, 0.3]]  # .3, .3
